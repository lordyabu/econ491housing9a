...
-   Before visualizing the data, we need to clean up some of the varaibles.
...
I can convert some variables to a numerical ordinal scale.

NA, Poor, Fair, TA, Good, Excellent ==> 0, 1, 2, 3, 4, 5

BsmtQual, BsmtCond, ExterQual, ExterCond, FireplaceQu, GarageQual, GarageCond, HeatingQC, KitchenQual, PoolQC
...
```
Now there are less null values. 
...
outliers <- function(data) {
  ...
  lower_bound <- Q[1] - 1.5 * IQR(data)
  upper_bound <- Q[2] + 1.5 * IQR(data)
...
remodboxplot <- ggplot(data = mydata, aes(x = Remodel_Group, y = SalePrice)) +
  geom_boxplot() +
  geom_point(data = mydata[outliers(mydata$SalePrice), ],
             aes(color = YearRemodAdd), 
...
```{r streetPave}
paved_data <- mydata[mydata$Street == "Pave", ]

paved_outliers <- outliers(paved_data$SalePrice)

 streetpaveviolinplot <- ggplot(data = paved_data, 
                               aes(x = factor(Street), 
                                   y = SalePrice)) +
  geom_violin(trim = FALSE) +  
  geom_point(data = paved_data[paved_outliers, ],
             aes(x = factor(Street), y = SalePrice, 
                 color = SalePrice), 
    position = position_jitter(width = 0.15), size = 1.5) +
  labs(x = "Paved", y = "Sale Price", 
       title = "Sale Price Distribution for Paved Street") +
  scale_color_viridis_c(name = "Sale Price") +
  scale_y_continuous(labels = point) +
  theme_minimal()
 
 streetpaveviolinplot

```
## Violin Plot for NW Ames Neighborhood
This is a continuation of the varaVroom model analysis
```{r NW Ames violin}
nwAmes_data <- mydata[mydata$Neighborhood == "NWAmes", ]

nwAmes_outliers <- outliers(nwAmes_data$SalePrice)

nwAmesviolinplot <- ggplot(data = nwAmes_data, 
                               aes(x = factor(Neighborhood), 
                                   y = SalePrice)) +
  geom_violin(trim = FALSE) +  
  geom_point(data = nwAmes_data[nwAmes_outliers, ],
             aes(x = factor(Neighborhood), y = SalePrice, 
                 color = SalePrice), 
    position = position_jitter(width = 0.15), size = 1.5) +
  labs(x = "NW Ames", y = "Sale Price", 
       title = "Sale Price Distribution for NW Ames Neighborhood") +
  scale_color_viridis_c(name = "Sale Price") +
  scale_y_continuous(labels = point) +
  theme_minimal()
